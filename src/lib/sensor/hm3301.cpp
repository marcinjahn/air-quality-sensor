...
namespace HM3301
{
    bool initialize()
    {
        I2C::initialize(SCL_FREQUENCY);

        return write_byte(SENSOR_WRITE_ADDRESS, INIT_COMMAND) == I2CStatusCode::data_byte_transmitted_with_ack;
    }

    Reading read()
    {
        uint8_t data[29] = { 0 };
        auto status = I2C::read(SENSOR_READ_ADDRESS, data, 29);

        Reading result;

        if (status != I2C::data_byte_received_without_ack)
        {
            result.is_ok = false;
        }
        else 
